...
function browser_load(proj, f, prod) {
...
var FTYPES = {
    ...
    coffee: {
        ...
        browser_load_prod: function(name) {
            return "<script type='" + name + "'></script>";
        }
    },
        ...
        build: function(args, callback) {
            ...
            FS.readFile(source, "utf8", function(err, code){
                ...
                code = code.replace(
                    ...
                    function(s, p1, p2) {
                        var deps = f.deps ? f.deps.slice(0) : [];
                        if (args.devel) deps.push("less");
                        deps = PROJECT.order_load_files(proj, deps).map(function(f){
                            return browser_load(proj, f, !args.devel);
                        });
                        return "<!--KENDO_BOOTSTRAPPER:FILES-->\n" +
                            deps.join("\n") +
                            p2 + "<!--/KENDO_BOOTSTRAPPER:FILES-->";
                    }
                );
                FS.writeFile(output, code, "utf8", function(err){
                    if (err) return callback(err);
...
function build_file(proj, f, devel, callback) {
