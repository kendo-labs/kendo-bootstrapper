...
RPC.defhandler("config/set-kendo-dir", function(request, dir){
    ...
    if (!has_core) return callback(null, false);
    var has_web = has_file("styles", "kendo.default.min.css");
    var has_mobile = has_file("styles", "kendo.mobile.common.min.css");
    if (!has_web && !has_mobile) return callback(null, false);
