...
function add_file(args, callback) {
    ...
    if (proj.files.contains(null, function(f){ return f.name == args.filename }))
        return callback(new Error("This file name is already registered in the project"));
...
function import_project(args, callback) {
    ...
    try {
        ...
        if (!stat.isDirectory())
            return callback(new Error(args.path + " is not a directory."));
    } catch(ex) {
        return callback(ex);
    }
    try {
...
exports.import_project = import_project;
...
RPC.defhandler("project/bootstrap", function(request, args){
    ...
    if (isEmpty) {
        ...
        import_project(proj, request.make_handler());
    }
});
