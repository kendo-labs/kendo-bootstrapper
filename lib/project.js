...
var KBUNDLE = require("kendo-lint/lib/bundle.js");
...
function project_kendo_widget_usage(proj_id, callback) {
    ...
    function fullname(f) {
        return PATH.join(proj.path, f.name);
    }
    var usage = KBUNDLE.widget_usage({
        js_files   : js_files.map(fullname),
        html_files : page_files.map(fullname),
        category   : proj.use_kendo,
    });
    callback(null, {
        widgets                 : usage.widgets,
        components              : usage.components,
        kendo_config            : usage.kendo_config,
...
function project_build_kendo(proj_id, args, callback) {
    ...
    save_meta(proj, function cont(err) {
        if (err) return callback(err);
        var code = KBUNDLE.build_kendo({
            components : selected,
            kendo_dir  : PATH.join(CONFIG.get("kendo_src_dir"), "js"),
...
RPC.defhandler("project/get-preview-url", function(request, proj_id){
    var url = "http://" + getIPAddress() + ":7569/@proj/" + encodeURIComponent(proj_id) + "/";
