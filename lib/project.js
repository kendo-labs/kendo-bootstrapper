...
function project_build_kendo(proj_id, load_components, callback) {
    var proj = get_project(proj_id);
    ...
    load_components.forEach(require);
    var f, added = false;
    for (var i = 0; i < proj.files.length; ++i) {
        if (proj.files[i].custom_kendo) {
            f = proj.files[i];
            break;
        }
    }
    function cont(err) {
        ...
        selection.forEach(function(filename){
            all += FS.readFileSync(PATH.join(TOPLEVEL_DIR, "kendosrc", "js", filename), "utf8");
        });
        FS.writeFile(PATH.join(proj.path, "kendo", "kendo-custom.js"), all, "utf8", function(err){
            if (err) return callback(err);
            if (added) {
                EVENTS.notify("project_add_file", {
                    proj_id      : proj.id,
