...
function project_rebuild_all(proj, build_type, callback) {
    ...
    STEP(function(){
        ...
        proj.files.forEach(function(f){
            if (f.page) {
                var callback = group();
                var code = build_page(proj, f, { devel: true });
                FS.writeFile(PATH.join(proj.path, f.name), code, function(err){
...
function build_page(proj, page, build_type) {
    ...
    var this_page_files = generate_load_files(proj, page, build_type);
...
function optimize_images(proj, callback) {
    proj = get_project(proj);
    try {
        var optipng = PLATFORM.get_tool("optipng");
        var jpegtran = PLATFORM.get_tool("jpegtran");
        var count = 0;
        UTILS.fs_find(proj.path, {
            skip_vcs: true,
            filter: function(f) {
