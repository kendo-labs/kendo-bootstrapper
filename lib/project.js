...
function register_project(proj, path) {
    ...
    (proj.watcher = CHOKIDAR.watch(path, { persistent: true }))
        .on("add", queue("add"))
    ...
    UTILS.fs_find(proj.path, {
        ...
        filter: function(f) {
            return !f.stat.isDirectory() &&
                (/\.(js|coffee|css|less|html|php|asp|jsp)$/i.test(f.rel));
        },
...
function build_page(proj, page, build_type) {
    ...
    var loads = this_page_files.map(function(f){
    ...
    if (build_type.autoreload) {
        loads.push("<script>/*<![CDATA[*/__THIS_PAGE_FILES__ = " +
                   ...
                   "/*]]*/</script>");
        loads.push("<script src=\"/js/autorefresh.js\"></script>");
    }
