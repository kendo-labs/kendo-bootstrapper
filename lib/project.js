...
function register_project(proj, path) {
    ...
    function queue(event) {
        ...
        return function(path) {
            ...
            files[path] = setTimeout(function(){
                delete files[path];
                var rel = PATH.relative(proj.path, path).sane_path();
    ...
    (proj._watcher = CHOKIDAR.watch(path, { persistent: true }))
        .on("add", queue("add"))
...
function bootstrap(args, callback) {
    ...
    UTILS.fs_ensure_directory(PATH.dirname(dest_dir), function(err){
        if (err) return callback(err);
        UTILS.fs_copytree(TEMPLATES.get_template_dir(args.template_id), dest_dir, {
            filter: function(f) {
...
function init_project_template(args, callback) {
    ...
    STEP(
        ...
        function process_metadata(err, content) {
            ...
            template_args.KENDO_THEMES.forEach(function(id){
                meta.files.push({
                    id   : "kendo." + id + ".css",
                    name : "kendo/css/kendo." + id + ".min.css",
                    type : "css",
                    lib  : true,
    ...
    function find_widget(name) {
        ...
        return kc.filter(function(comp){
            return comp.widgets.contains(null, function(el){
                // XXX: the way we discard the mobile is WAT.
                //      this should depend on the project type, probably, when we add support for that.
                if (el.name.toLowerCase() == name && !/kendo\.mobile/.test(el.file)) {
...
function load_assets(proj, page) {
    proj = get_project(proj);
    page = get_file_by_name(proj, page);
    return "document.write(" + JSON.stringify(assets()) + ")";
    ...
    function assets() {
        ...
        loads.push("<script>/*<![CDATA[*/__THIS_PAGE_FILES__ = " +
                   JSON.stringify(this_page_files, null, 2) + ";\n" +
                   "__THIS_PAGE__ = " + JSON.stringify(page, null, 2) +
                   "/*]]*/</script>");
        loads.push("<script src=\"http://localhost:7569/js/rpc.js\"></script>");
        loads.push("<script src=\"http://localhost:7569/js/autorefresh.js\"></script>");
        return loads.join("\n");
    }
};
